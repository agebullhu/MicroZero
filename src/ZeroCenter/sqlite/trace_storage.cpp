...
namespace agebull
{
	namespace zero_net
	{
		...
		bool trace_storage::prepare_storage()
		{
			...
			tm day = *(localtime(&tt));
			acl::string path;
			path.format("%s/datas/zero_trace.%04d%02d%02d.db"
			...
			try_create("tb_trace", trace_create_sql_,
			           {
				           "CREATE INDEX[main].[trace_id] ON[tb_trace]([id]);",
				           "CREATE INDEX[main].[trace_global_id] ON[tb_trace]([global_id]);",
				           "CREATE INDEX[main].[trace_call_id] ON[tb_trace]([call_id]);",
				           "CREATE INDEX[main].[trace_call_id] ON[tb_trace]([call_id]);"
			           });
			try_create("tb_frames", frame_create_sql_,
			           {
				           "CREATE INDEX[main].[frames_id] ON[tb_frames]([id]);",
				           "CREATE INDEX[main].[frames_trace_id] ON[tb_frames]([trace_id]);",
				           "CREATE INDEX[main].[frames_index] ON[tb_frames]([index]);"
			           });

			if (sqlite3_prepare_v2(sqlite_db_, trace_insert_sql_, static_cast<int>(strlen(trace_insert_sql_)),
			                       &trace_insert_stmt_, nullptr) != SQLITE_OK)
			{
				log_error2("[%s] : db > Can't open tb_trace :%s", name_, sqlite3_errmsg(sqlite_db_));
				return false;
			}
			if (sqlite3_prepare_v2(sqlite_db_, trace_last_sql_, static_cast<int>(strlen(trace_last_sql_)),
			                       &trace_last_stmt_, nullptr) != SQLITE_OK)
			{
				log_error2("[%s] : db > Can't open tb_trace :%s", name_, sqlite3_errmsg(sqlite_db_));
				return false;
			}
			if (sqlite3_prepare_v2(sqlite_db_, frame_insert_sql_, static_cast<int>(strlen(frame_insert_sql_)),
			                       &frame_insert_stmt_, nullptr) != SQLITE_OK)
			{
		...
		bool trace_storage::save_trace(int in_out, const char* reqid, const int64 par_id, const int64 my_id,
		                               const char* station_name, const char* station_type, const char* state_str,
		...
		*\brief 将数据写入数据库中
		*/
		void trace_storage::save(int in_out, const char* reqid, const int64 par_id, const int64 my_id,
		                         const char* station_name, const char* station_type, const char* state_str,
		                         vector<shared_char>& frames)
		{
			time_t tt = time(nullptr);
			...
			begin_trans();
			save_trace(in_out, reqid, par_id, my_id, station_name, station_type, state_str, frames);
			commit_trans();
		}
	}
}
