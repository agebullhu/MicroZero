...
namespace ZeroNet.Http.Route
{
    ...
    public class RouteData
    {
        ...
        /// </summary>
        [DataMember] [JsonProperty("bear")] public string Bearer;
        ...
        ///     执行状态
        /// </summary>
        [DataMember] [JsonProperty("status")] public RouteStatus Status;
        ...
        /// </summary>
        [DataMember]
        [JsonProperty("succeed")] public bool IsSucceed => Status == RouteStatus.None;
        ...
        {
            Uri = request.GetUri();
            foreach (var head in request.Headers) Headers.Add(head.Key, head.Value.ToList());
            HttpMethod = request.Method.ToUpper();
            if (request.QueryString.HasValue) Form = request.QueryString.ToString();
            if (request.HasFormContentType) Form = request.Form.LinkToString(p => $"{p.Key}={p.Value}", "&");
