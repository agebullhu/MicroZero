...
#pragma warning disable CS1591 // 缺少对公共可见类型或成员的 XML 注释
        ...
        ///     实例
        /// </returns>
        protected ZSocket(ZContext context, ZSocketType socketType, out ZError error)
        {
            Context = context;
        ...
        public void Close()
        {
            if (!Close(out Error)) throw new ZException(Error);
        }
        ...
        {
            if (!Bind(endpoint, out Error)) throw new ZException(Error);
        }
        ...
        {
            if (!Unbind(endpoint, out Error)) throw new ZException(Error);
        }
        ...
        {
            if (!Connect(endpoint, out Error)) throw new ZException(Error);
        }
        ...
        public ZMessage ReceiveMessage(ZSocketFlags flags)
        {
            ...
            if (!Equals(Error, ZError.None)) throw new ZException(Error);
            return message;
        }
        ...
        {
            if (!SendBytes(buffer, offset, count, ZSocketFlags.None, out Error)) throw new ZException(Error);
            return true;
        }
        ...
        */
        ...
        {
            if (!SendMessage(msg, flags, out Error)) throw new ZException(Error);
        }
        ...
        {
            if (!SendFrame(frame, flags, out Error)) throw new ZException(Error);
        }
        ...
        {
            return Error = ZError.GetLastErr();
        }
        ...
        protected bool SendFrame(ZFrame frame, int flags)
        {
            if (zmq.msg_send(frame.Ptr, SocketPtr, flags) >= 0)
                return true;
            Error = ZError.GetLastErr();
            LogRecorderX.Error($"Send Error: {Error.Text} | {Endpoint} | Socket Ptr:{SocketPtr}");
            return false;
        }
        ...
        protected bool RecvFrame(out ZFrame frame, int flags)
        {
            ...
            Error = ZError.GetLastErr();
            frame.Dispose();
            LogRecorderX.Error($"Recv Error: {Error.Text} | {Endpoint} | {SocketPtr}");
            return false;
        }
        ...
        private bool CloseInner(out ZError error)
        {
            ...
            try
            {
                ...
                if (SocketPtr != IntPtr.Zero)
                {
                    ...
                    {
                        error = Error = ZError.GetLastErr();
                        LogRecorderX.Error($"Socket Close Error:{Error}");
                        State |= SocketState.Failed;
                        success = false;
                    }
                }
            ...
            using (var optionLengthP = DispoIntPtr.Alloc(IntPtr.Size))
            {
                switch (IntPtr.Size)
                {
                    ...
                    case 8:
                        ...
                        break;
                    default:
                        Error = error = new ZError(ZError.Code.NOSUPPORT);
                        return false;
                }
